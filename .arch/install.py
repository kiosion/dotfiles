...
# TODO: Menu opt to symlink dotfiles from github
# would need to be run post-install and post-user-creation
...
# Log various information about hardware before starting the installation.
# This might assist in troubleshooting
archinstall.log(
  f"Hardware model detected: {archinstall.sys_vendor()} \
{archinstall.product_name()}; UEFI mode: {archinstall.has_uefi()}",
  level=logging.DEBUG
)
archinstall.log(
  f"Processor model detected: {archinstall.cpu_model()}",
  level=logging.DEBUG
)
archinstall.log(
  f"Memory statistics: {archinstall.mem_available()} available out of \
{archinstall.mem_total()} total installed",
  level=logging.DEBUG
)
archinstall.log(
  f"Virtualization detected: {archinstall.virtualization()}; is VM: \
    {archinstall.is_vm()}",
  level=logging.DEBUG
)
archinstall.log(
...
# For support reasons, we'll log the disk layout pre installation to match
# against post-installation layout
archinstall.log(
...
def ask_user_questions():
    ...
    # Ask which boot-loader to use (will only ask if we're in UEFI mode,
    # otherwise will default to GRUB)
    ...
    # Ask for a root password (optional, but triggers requirement
    # for super-user if skipped)
...
def perform_filesystem_operations():
    ...
    if archinstall.arguments.get('harddrives', None):
        print(
          _(f" ! Formatting {archinstall.arguments['harddrives']} in "),
          end=''
        )
...
def perform_installation(mountpoint):
    ...
    with archinstall.Installer(
        ...
        # This *can* be done outside of the installation, but the installer
        # can deal with it.
        ...
        # Placing /boot check during installation because this will catch
        # both re-use and wipe scenarios.
        for partition in installation.partitions:
            if partition.mountpoint == installation.target + '/boot':
                # ~200 MiB in GiB
                if partition.size < 0.19:
                    raise archinstall.DiskError(
                      'The selected /boot partition in use is not large \
enough to properly install a bootloader. Please resize it to at least \
200MiB and re-run the installation.'
                    )

        # if len(mirrors):
        # Certain services might be running that affects the system
        # during installation. Currently, only one such service is
        # "reflector.service" which updates /etc/pacman.d/mirrorlist
        # We need to wait for it before we continue since we
        # opted in touse a custom mirror/region.
        installation.log(
        ...
        # make sure at least one time-sync finishes before we continue with
        # the installation
        if archinstall.arguments.get('ntp', False):
            # Activate NTP in the ISO
            archinstall.SysCommand('timedatectl set-ntp true')

            # TODO: This block might be redundant, but this service
            # is not activated unless `timedatectl set-ntp true` is executed.
            logged = False
            while archinstall.service_state(
              'dbus-org.freedesktop.timesync1.service'
                ...
                if not logged:
                    installation.log(
                      'Waiting for dbus-org.freedesktop.timesync1.service \
to enter running state',
            ...
            while 'Server: n/a' in archinstall.SysCommand(
              'timedatectl timesync-status --no-pager --property=Server \
--value'
            ):
                if not logged:
                    installation.log(
                      'Waiting for timedatectl timesync-status to report \
a timesync against a server',
                      level=logging.INFO
                    )
                    logged = True
                time.sleep(1)
        ...
        # Retrieve list of additional repositories and
        ...
        if installation.minimal_installation(
                ...
                locales=[
                  f"{archinstall.arguments['sys-language']} \
{archinstall.arguments['sys-encoding'].upper()}"
            ...
            if archinstall.arguments.get(
              "bootloader"
            ) == "grub-install" and archinstall.has_uefi():
            ...
            if network_config:
                handler = NetworkConfiguration.NetworkConfigurationHandler(
                  network_config
                )
            ...
            if archinstall.arguments.get('audio', None) is not None:
                installation.log(
                  f"This audio server will be used: \
{archinstall.arguments.get('audio', None)}",
                  level=logging.INFO
                )
            ...
            if (root_pw := archinstall.arguments.get(
              '!root-password',
              None
            )) and len(root_pw):
                installation.user_set_pw('root', root_pw)

            # This step must be after profile installs to allow profiles to
            # install language pre-requisits. After which, this step will
            # set the language both for console and x11 if x11 was
                ...
                with archinstall.arguments['profile'].load_instructions(
                  namespace=f"{archinstall.arguments['profile'].namespace}.py"
                    ...
                    if not imported._post_install():
                        archinstall.log(
                          ' * Profile\'s post configuration requirements \
was not fulfilled.',
                          fg='red'
                        )
                        exit(1)
        ...
        # Note that while it's called enable_service, it can actually
        # take a list of services and iterate it.
...
and perform post-installation configuration?'
    ...
    # For debug, log the disk layout post-installation
    # (crash or no crash)
    archinstall.log(
...
if archinstall.arguments.get(
  'skip-mirror-check',
  False
) is False and archinstall.check_mirror_reachable() is False:
    log_file = os.path.join(
      archinstall.storage.get('LOG_PATH', None),
      archinstall.storage.get('LOG_FILE', None)
    )
    archinstall.log(
      f"Arch Linux mirrors are not reachable. Please check your internet \
connection and the log file '{log_file}'.",
      level=logging.INFO,
      fg="red"
    )
    exit(1)
...
if not archinstall.arguments.get('offline'):
    ...
    if archinstall.arguments.get('skip-keyring-update', False) is False and \
        archinstall.installed_package(
          'archlinux-keyring').version < latest_version_archlinux_keyring:
        ...
        if not archinstall.update_keyring():
            ...
            archinstall.log(
              f"Failed to update the keyring. Please check your internet \
connection and the log file '{log_file}'.",
              level=logging.INFO,
              fg="red"
            )
